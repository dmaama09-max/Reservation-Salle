...
{
    private const DUREE_MAXIMALE_EN_HEURES = 4;
    ...
    public function creer(CreerReservationDTO $dto): Reservation
    {
        ...
        if ($salle === null) {
            throw new SalleIndisponibleException("Cette salle n'existe pas.");
        }
        ...
        $dureeEnHeures = $dureeEnSecondes / 3600;

        if ($dureeEnHeures > self::DUREE_MAXIMALE_EN_HEURES) {
        ...
        $conflit = $this->reservations->trouverConflit(
            $dto->salleId,
            $dto->dateDebut,
            $dto->dateFin
        );
